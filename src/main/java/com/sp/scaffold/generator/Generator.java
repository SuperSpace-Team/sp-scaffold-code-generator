...
import com.sp.scaffold.generator.util.FileHelper;
import com.sp.scaffold.generator.util.GLogger;
import com.sp.scaffold.generator.util.IOHelper;
...
public class Generator {
	...
	private List<Exception> generateBy(File templateRootDir, Map templateModel, Map filePathModel) throws Exception {
		...
		System.out.println("-------------------load template from templateRootDir = '"
		...
		for (int i = 0; i < templateFiles.size(); i++) {
			...
			if ((testExpressionIndex = templateRelativePath.indexOf('@')) != -1) {
				...
				if (!"true".equals(String.valueOf(expressionValue))) {
					System.out.println("[not-generate]\t test expression '@" + testExpressionKey
							+ "' is false,template:" + templateRelativePath);
					continue;
				}
			}
			...
			try {
				...
				generateNewFileOrInsertIntoFile(templateModel, targetFilename, newFreeMarkerConfiguration(),
				...
				if (ignoreTemplateGenerateException) {
					...
					throw new RuntimeException("generate oucur error,templateFile is:" + templateRelativePath + " => "
							+ targetFilename, e);
				}
			}
		}
	...
	private String getTargetFilename(Map filePathModel, String templateFilepath) throws Exception {
		...
		try {
			...
			throw new IllegalStateException("cannot generate filePath from templateFilepath:" + templateFilepath
					+ " cause:" + e, e);
		}
	}
